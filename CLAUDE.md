...
- The textarea accepts multi-line Hebrew input. On **הצג על הלוח** (or `Ctrl/Cmd+Enter`), `renderMessage()` rebuilds the flap grid sized to the longest line.
...
- The size slider updates the CSS custom property `--board-scale`, applied via `transform: scale()` on `.board`.
